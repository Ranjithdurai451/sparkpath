    ...
    const fetchData = async () => {
      try {
        if (!localStorage.getItem(`${itemTitle}-explaination`)) {
          ...
          const result = await generateRoadmapExplaination({
            formData,
            taskTitle: itemTitle,
          });
          ...
          setItemData(
            JSON.parse(
              localStorage.getItem(`${itemTitle}-explaination`) || "{}"
            )
          );
        }
      } catch (error) {
          ...
          {itemData ? (
              ...
              <CardContent className="p-6">
                ...
                <div className="space-y-3 mb-4">
                  {itemData?.explaination?.howToDoItRight.map((item, index) => (
                    <div
                      key={index}
                      className="cursor-pointer p-4 border border-secondary rounded-sm"
                ...
                <div className="space-y-2 mt-4">
                  {itemData?.explaination?.resources.map((item, index) => (
                    <div
                      key={index}
                      className="flex pl-2 gap-2 items-center justify-between"
                    >
            ...
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground">
                The requested item explanation is not available.
              </p>
            </div>
          )}
        </div>
