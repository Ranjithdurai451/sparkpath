...
import { generateFailurePrediction } from '@/lib/action';
...
const RiskIndicator = ({ score }: { score: number }) => {
  ...
  return (
    <div className="flex items-center">
      ...
      <div
        className={`ml-3 flex items-center ${textColor} text-sm font-medium`}
      >
        {icon}
        {label}
      </div>
...
const FailurePrediction = () => {
  ...
  useEffect(() => {
    ...
    const storedFailurePrediction = localStorage.getItem('failure-prediction');
    if (!storedFormData || !storedFailurePrediction) {
      return;
    }
    if (storedFormData && !storedFailurePrediction) {
      ...
      const parsedData = JSON.parse(storedFailurePrediction);
      setData(parsedData);
    }
  }, []);
  ...
  const riskAreas = [
    {
      name: 'Market Fit',
      score: insights.marketFit.percentage,
      details: insights.marketFit.suggestion,
    },
    {
      name: 'Cash Flow',
      score: insights.cashFlow.percentage,
      details: insights.cashFlow.suggestion,
    },
    {
      name: 'Team Composition',
      score: insights.teamComposition.percentage,
      details: insights.teamComposition.suggestion,
    },
    {
      name: 'Competition',
      score: insights.competition.percentage,
      details: insights.competition.suggestion,
    },
    {
      name: 'Scalability',
      score: insights.scalability.percentage,
      details: insights.scalability.suggestion,
    },
  ];
  ...
  return (
    <Card className="glass-panel border-none shadow-lg animate-scale-in overflow-hidden hover:shadow-xl transition-all duration-300">
      <CardHeader className="border-b border-border">
        ...
        <CardDescription>
          AI-powered assessment of potential risks and challenges for your
          startup
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="p-6">
          ...
          <div className="relative h-3 overflow-hidden rounded-full bg-secondary">
            <div
              className={`absolute left-0 top-0 h-full transition-all duration-500 ${
                parseFloat(failure_rate) > 70
                  ? 'bg-red-500'
                  : parseFloat(failure_rate) > 40
                  ? 'bg-amber-500'
                  : 'bg-green-500'
              }`}
        ...
        <div className="bg-secondary/50 rounded-lg p-4 mb-6">
          ...
          <div className="relative h-3 overflow-hidden rounded-full bg-secondary">
            <div
              className={`absolute left-0 top-0 h-full transition-all duration-500 ${
                parseFloat(success_rate) > 70
                  ? 'bg-red-500'
                  : parseFloat(success_rate) > 40
                  ? 'bg-amber-500'
                  : 'bg-green-500'
              }`}
        ...
        <div className="space-y-5">
          <h3 className="text-lg font-bold">
            Risk Factors From Our RandomForest Model
          </h3>
        ...
        <div className="space-y-5">
          {riskAreas.map((area, index) => (
            <div
              key={index}
              className="animate-fade-in hover:translate-x-1 transition-transform duration-300"
