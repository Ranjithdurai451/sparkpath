...
const CompetitorAnalysis = () => {
  ...
  useEffect(() => {
    ...
    let storedSwotData = localStorage.getItem('swot');
    if (!storedFormData || !storedSwotData) {
      return;
    }
    if (storedFormData && !swotData) {
      const formData = JSON.parse(storedFormData);
      generateSWOTAnalysis(formData)
        .then((response) => {
          setSwotData(response);
          localStorage.setItem('swot', JSON.stringify(response.data));
        })
        .catch((error) => {
          console.error('Error fetching data:', error);
        });
    } else {
      const parsedData = JSON.parse(storedSwotData);
  ...
  return (
    <Card className="glass-panel border-none shadow-lg animate-scale-in overflow-hidden">
      <CardHeader className="border-b border-border">
        ...
        <CardDescription>
          Evaluate your competition's strengths, weaknesses, opportunities, and
          threats
        </CardDescription>
      </CardHeader>
              ...
              <div className="space-y-2">
                ...
                <ul className="space-y-2">
                  {competitor?.strengths?.map((strength, sIndex) => (
                    <li
                      key={sIndex}
                      className="bg-green-50 dark:bg-green-950/30 p-3 rounded-lg text-sm flex items-start"
                    >
              ...
              <div className="space-y-2">
                ...
                <ul className="space-y-2">
                  {competitor?.weaknesses?.map((weakness, wIndex) => (
                    <li
                      key={wIndex}
                      className="bg-red-50 dark:bg-red-950/30 p-3 rounded-lg text-sm flex items-start"
                    >
              ...
              <div className="space-y-2">
                ...
                <ul className="space-y-2">
                  {competitor?.opportunities?.map((opportunity, oIndex) => (
                    <li
                      key={oIndex}
                      className="bg-blue-50 dark:bg-blue-950/30 p-3 rounded-lg text-sm flex items-start"
                    >
              ...
              <div className="space-y-2">
                ...
                <ul className="space-y-2">
                  {competitor?.threats?.map((threat, tIndex) => (
                    <li
                      key={tIndex}
                      className="bg-amber-50 dark:bg-amber-950/30 p-3 rounded-lg text-sm flex items-start"
                    >
